...
function createScopedContainer(scope) {
    warning(typeof getComputedStyle !== "function" ||
...
function resolveTileTransition(transition, count, spread = DEFAULT_STAGGER_WINDOW) {
    ...
    if (resolved.delay === undefined && count > 1) {
        ...
        resolved.delay = stagger(spread / count);
    }
    return resolved;
}
/**
...
function wipe(options = {}) {
    ...
    warning(isHorizontal(cover) === isHorizontal(reveal), "curtains: wipe can't reveal on a different axis than it covers; the reveal direction is ignored");
    ...
    return {
        ...
        cover(transition) {
            return animate(panel, { transform: [coverFrom, coverTo] }, resolveTransition(transition));
        },
        reveal(transition) {
            return animate(panel, { transform: [coverTo, revealTo] }, resolveTransition(transition));
        },
    };
}
...
function clipWipe(options = {}) {
    ...
    const { cover, reveal } = resolveDirections(direction, directionMode);
    warning(isHorizontal(cover) === isHorizontal(reveal), "curtains: clipWipe can't reveal on a different axis than it covers; the reveal direction is ignored");
    warning(!(bow > 0 && angle !== 0), "curtains: clipWipe ignores `angle` when `bow` is set");
    ...
    return {
        ...
        cover(transition) {
            return animate(overlay, { clipPath: [frames.coverFrom, frames.coverTo] }, resolveTransition(transition));
        },
        reveal(transition) {
            return animate(overlay, { clipPath: [frames.revealFrom, frames.revealTo] }, resolveTransition(transition));
        },
    };
}
...
function doors(options = {}) {
    ...
    return {
        ...
        cover(transition) {
            return animate(panels, { transform: [`${scale}(0)`, `${scale}(1)`] }, resolveTransition(transition));
        },
        reveal(transition) {
            return animate(panels, { transform: [`${scale}(1)`, `${scale}(0)`] }, resolveTransition(transition));
        },
    };
}
        ...
        reveal(transition) {
            if (directional) {
                ...
                const out = directionMode === "reverse"
                    ? fractions.map((f) => 1 - f)
                    : fractions;
                return animate(sequence, { opacity: [1, 0] }, tileDelayTransition(transition, out));
            }
            const out = directionMode === "reverse"
                ? sequence.slice().reverse()
                : sequence;
            return animate(out, { opacity: [1, 0] }, resolveTileTransition(transition, out.length));
        },
    };
}
...
function scaleStripsEffect(config) {
    ...
    return {
        ...
        cover(transition) {
            return animate(strips, { transform: [hidden, shown] }, resolveTileTransition(transition, strips.length));
        },
        reveal(transition) {
            for (const strip of strips) {
            ...
            return animate(out, { transform: [shown, hidden] }, resolveTileTransition(transition, out.length));
        },
    };
}
